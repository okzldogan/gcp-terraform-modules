  ...
  settings {
    ...
    dynamic "ip_configuration" {
      ...
      content {
        ...
        enable_private_path_for_google_cloud_services = lookup(ip_configuration.value, "enable_private_path", false)
...
resource "google_sql_user" "default" {
  ...
  password_policy {
    allowed_failed_attempts      = var.default_user_allowed_failed_attempts
    enable_failed_attempts_check = var.default_user_enable_failed_attempts_check
  }
  depends_on = [
...
resource "google_sql_user" "additional_users" {
  ...
  project  = var.project_id
  name     = each.value.name
  password = each.value.random_password ? random_password.temporary_password_for_additional_users[each.value.name].result : null
  ...
  depends_on = [
    google_sql_database_instance.default,
    google_sql_database_instance.replicas,
    random_password.temporary_password_for_additional_users,
  ]
}

resource "random_password" "temporary_password_for_additional_users" {
