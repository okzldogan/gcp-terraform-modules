...
resource "google_sql_database_instance" "replicas" {
  ...
  replica_configuration {
    failover_target = var.read_replica_failover
  }
  ...
  settings {
    ...
    dynamic "backup_configuration" {
      for_each = [lookup(each.value, "backup_configuration", {})]

      content {

        enabled                         = lookup(backup_configuration.value, "enabled", false)
        binary_log_enabled              = lookup(backup_configuration.value, "binary_log_enabled", true)
        start_time                      = lookup(backup_configuration.value, "start_time", null)
        location                        = lookup(backup_configuration.value, "location", null)
        transaction_log_retention_days  = lookup(backup_configuration.value, "transaction_log_retention_days", 7)
