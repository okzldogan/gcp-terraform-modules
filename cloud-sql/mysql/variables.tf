...
variable "edition" {
  type        = string
  description = "Edition type of the Cloud SQL resources"
  default     = "ENTERPRISE"
}
...
variable "additional_user_passwords" {
  type        = list(map(any))
  default     = []
  sensitive   = true
}

variable "default_user_allowed_failed_attempts" {
  type        = number
  description = "Number of allowed failed attempts for default/root user."
  default     = 5
}

variable "default_user_enable_failed_attempts_check" {
  type        = bool
  description = "Set true to enable failed attempts check"
  default     = true
}

variable "read_replica_failover" {
