...
resource "google_compute_health_check" "http_health_check" {
    ...
    http_health_check {
        ...
        port               = lookup(each.value, "health_check_port", 3000) 
...
resource "google_compute_backend_service" "backend" {
    ...
    backend {
        group = "https://www.googleapis.com/compute/v1/projects/${var.project_id}/zones/${each.value.neg_zone_2}/networkEndpointGroups/${each.value.neg_name}"
        balancing_mode = "RATE"
        max_rate_per_endpoint = 1.0
        capacity_scaler = 1.0
